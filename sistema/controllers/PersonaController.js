...
export default {
    ...
    update: async (req, res, next) => {
        ...
        try {
            ...
            const reg = await models.Persona.findByIdAndUpdate({ _id: req.body._id }, { tipo_persona: req.body.tipo_persona, nombre: req.body.nombre, email: req.body.email, telefono: req.body.telefono, direccion: req.body.direccion });
            res.status(200).json(reg);
    ...
    activate: async (req, res, next) => {
        ...
        try {
            const reg = await models.Persona.findByIdAndUpdate({ _id: req.body._id }, { estado: 1, listaNegra: 0 });
            res.status(200).json(reg);
        } catch (e) {
            res.status(500).send({
                message: 'Ocurrio un error'
    ...
    deactivate: async (req, res, next) => {
        ...
        try {
            const reg = await models.Persona.findByIdAndUpdate({ _id: req.body._id }, { estado: 0, listaNegra: 1 });
            res.status(200).json(reg);
