...
class Purchases extends Component {
    ...
    constructor(props) {
        ...
        this.headers = [
            ...
            ['Nombre', 'Correo electrónico', 'Teléfono', 'Dirección', 'Estado']];
        this.ids = [
                            ...
                            fields={
                                {
                                    ...
                                    serie_comprobante: { type: 'TextField', label:'Serie Comprobante' },
                                    num_comprobante: { type: 'TextField', label: 'Número de Comprobante' },
                                    impuesto: { type: 'TextFieldNumericImpuesto', label:'Impuesto' },
                                    detalles: { type: 'ShopList', options: this.state.data[2] ? this.state.data[2].map(el => ({ id: el._id, value: el.nombre, precio: el.precio_venta, cantidadTotal:1000 })) : [] , label:'Detalles'}
                                }
                            }
                            key="tab1" />
                        <TableConsult
                            ...
                            fields={
                                {
                                    ...
                                    num_documento: { type: "TextField", label: 'Número de Comprobante' },
    ...
    handleSwitchActivate(id) {
        const pathQuery = this.state.activeTab === 0 ? '/ingreso/query?_id=' : '/persona/query?_id=';
        this.Requests.query(pathQuery + id).then(response => {
            //Actualizar tabla activa
            const pathActDeactivate = (this.state.activeTab === 0 ? '/ingreso/' : '/persona/') + ((response.estado && response.estado == 1) ? "deactivate" : "activate");
